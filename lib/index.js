...
/*
 * This function adds utility methods related to setting timeouts
 */
function exposeGlobalTimers() {
 ...
 */
function setRequestsTimeout(config) {
    ...
    timeout = config.inc_req_timeout || config.incoming_timeout;
    if (timeout > 0) {
        process.setIncomingRequestsTimeout(timeout);
    }
...
function instrumentReq(config, req, resp) {
    ...
    setRequestsTimeout(config);
    if (globalIncoming) {
        closer = new Closer(resp, globalIncoming);
    }
}
